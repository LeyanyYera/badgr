...
from .models import Issuer, BadgeClass, BadgeInstance, FamilyClass, CategoryClass
...
class FamilyClassAdmin(ModelAdmin):
    readonly_fields = ('created_at', 'created_by')
...
badgr_admin.register(FamilyClass, FamilyClassAdmin)


class CategoryClassAdmin(ModelAdmin):
    readonly_fields = ('created_at', 'created_by')
    ...
    fieldsets = (
        ('Metadata', {'fields': ('created_by', 'created_at',), 'classes': ("collapse",)}),
        (None, {'fields': ('name', 'slug', 'family')}),
        ('JSON', {'fields': ('json',)}),
    )

badgr_admin.register(CategoryClass, CategoryClassAdmin)
