...
from mainsite.models import (AbstractIssuer, AbstractBadgeClass,
                             AbstractBadgeInstance, AbstractFamilyClass, AbstractCategoryClass)
...
class FamilyClass(AbstractFamilyClass):

    def publish(self):
        super(FamilyClass, self).publish()
        # self.issuer.publish()
        self.publish_by('slug')


class CategoryClass(AbstractCategoryClass):
    family = models.ForeignKey(FamilyClass, blank=False, null=False,
                               on_delete=models.CASCADE,
                               related_name="categoryclasses")

    def publish(self):
        super(CategoryClass, self).publish()
        # self.issuer.publish()
        self.publish_by('slug')