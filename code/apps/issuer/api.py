...
class BadgeInstanceList(AbstractIssuerAPIEndpoint):
    ...
    def post(self, request, issuerSlug, badgeSlug):
        ...
        if not self.validateDate(request.data.get('expires')) and request.data.get('expires') is not None:
            return Response(
                {"code": 1002, "description": "Date format is incorrect"},
                status=status.HTTP_400_BAD_REQUEST
            )

        badgeclass_queryset = self.queryset.filter(issuer__slug=issuerSlug)
        # Validate issuer has adequate permissions over badge
        if len(badgeclass_queryset) == 0:
            return Response(
                {"error":1005, "description": "BadgeClass not found or inadequate permissions."},
                status=status.HTTP_404_NOT_FOUND
            )

        # Get issuer by slug
        issuer = Issuer.objects.filter(slug=issuerSlug)
        # Verify if the issuer exits
        if len(issuer) == 0:
            return Response(
                {"error": 1005, "description": "Issuer not found"},
                status=status.HTTP_404_NOT_FOUND
            )

        # Get badge by slug
        badge = BadgeClass.objects.filter(slug=badgeSlug)
        # Verify if the badge exits
        if len(badge) == 0:
            return Response(
                {"error": 1006, "description":"Badge not found"},
                status=status.HTTP_404_NOT_FOUND
            )

        current_badgeclass = self.get_object(badgeSlug, queryset=badgeclass_queryset)

        # Get badge by recipient_identifier and badgeclass_id when the request is through the Badgr
        badge_instances_web = BadgeInstance.objects.filter(recipient_identifier=request.data.get('email'),
                                                           badgeclass_id=current_badgeclass.id,
                                                           revoked=False)
        # Get badge by recipient_identifier and badgeclass_id when the request is through the API
        badge_instances_api = BadgeInstance.objects.filter(
            recipient_identifier=request.data.get('recipient_identifier'), badgeclass_id=current_badgeclass.id, revoked=False)
        ...
        # Verify if the request is through the Badgr and if the medal was already assigned to the user
        if request.data.get('email') is not None and len(badge_instances_web) > 0:
            return Response(
                {"error": 1004, "description": "The medal was already issued to the recipient"},
                status=status.HTTP_400_BAD_REQUEST
            )

        # Verify if the request is through the API and if the medal was already assigned to the user
        if request.data.get('recipient_identifier') is not None and len(badge_instances_api) > 0:
            return Response(
                {"error": 1004, "description": "The medal was already issued to the recipient"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if request.data.get('expires') is not None:
            data['expires'] = \
                request.data.get('expires')
